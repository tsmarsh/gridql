...
before(async function () {
    ...
    let swagger_docs = await Promise.all(config.restlettes.map(async (restlette) => {
        let response = await fetch("http://localhost:3033" + restlette.path + "/api-docs/swagger.json")
...
async function buildApi(swagger_docs) {
    let apis = await Promise.all(swagger_docs.map(async (doc) => {
